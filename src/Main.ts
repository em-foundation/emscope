...
CMD.command('view')
    ...
    .option('-j --jls-file [optional event ID]', 'generate a Joulescope .jls file containing events')
    .option('-s --sleep-info', 'characterize power consumption when inactive')
    .option('-w --what-if [event-cycle duration]', `extrapolate results at a given event rate (default: '00:00:01')`, Core.parseHms)
